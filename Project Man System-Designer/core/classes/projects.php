...
public function project_progress($id) 
	...
	{
		$query = $this->db->prepare("SELECT SUM(value) AS tot_amount FROM selected_check WHERE site_id= ? AND c_status = 'Done' ");
		$query->bindValue(1,$id);
...
public function get_features($id) 
	...
	{
		$query = $this->db->prepare("SELECT * FROM selected_check T1 INNER JOIN checklisst T2 ON T1.check_id = T2.id WHERE T1.site_id = ?  ");
		$query->bindValue(1,$id);
...
public function checklist_done($id,$check_id) {
		...
		$query = $this->db->prepare("UPDATE selected_check SET c_status = ?, value = ? WHERE site_id = ? AND check_id = ?");
...
public function get_checklist_done($id) 
	...
	{
		$query = $this->db->prepare("SELECT * FROM selected_check WHERE site_id =? ");
		$query->bindValue(1,$id);
...
public function complete_enabled($id) 
	...
	{
		$query = $this->db->prepare("SELECT * FROM selected_check T1 INNER JOIN site_data T2 ON T2.id =T1.site_id WHERE site_id = ? AND c_status='Done'");
		$query->bindValue(1,$id);
