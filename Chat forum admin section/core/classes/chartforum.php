...
/*=========================Insert comment by tello admin=========================================*/
	public function admin_insert_comments($m_id,$admin_id,$reply)
	{

	  $query = $this->db->prepare("INSERT INTO `comments`(`m_id`,`admin_id`,`reply`) VALUES (?,?,?)");
	...
	{
            $query = $this->db->prepare("SELECT T1.reply,T2.username,T1.likes,T1.id,T1.m_id,T3.email,T1.designer_id,T1.admin_id FROM comments T1 LEFT JOIN designer T2 ON T1.designer_id = T2.id lEFT JOIN admin T3 ON T1.admin_id = T3.id WHERE T1.m_id = ? ORDER BY T1.likes DESC");
...
/*==================insert into likes comment for counting====================================*/
	...
	public function delete_notifications($m_id,$designer_id)
	  ...
	  public function list_my_messages($designer_id)
	{
      $query = $this->db->prepare("SELECT DISTINCT T1.id,T1.designer_id,T1.message,T1.category,T3.m_id,T3.status FROM chat_forum T1 INNER JOIN designer T2 ON T1.designer_id = T2.id INNER JOIN comments T3 ON T1.id = T3.m_id WHERE T1.designer_id = ? ORDER BY T3.status DESC");
