...
public function all_complited_ad(){
		...
		$query	= $this->db->prepare("SELECT  DISTINCT  t1.username , t1.contacts ,t2.user_id FROM users t1 INNER JOIN client_payment t2 ON  t1.id = t2.user_id WHERE t2.client_status = 'Completed'  ");
	...
	public function all_complited_ad11($user_id){
		global $db;

		$query	= $this->db->prepare("SELECT  DISTINCT   t1.username , t1.contacts ,t2.user_id ,t2.payment_method ,t2.payment_type ,t2.amount ,t2.date FROM users t1 INNER JOIN client_payment t2 ON  t1.id = t2.user_id  INNER JOIN tbl_uploads t3 ON t3.c_id = t2.c_id WHERE t2.user_id =? AND t2.client_status = 'Completed'  ");
	...
	public function all_Completed_ad(){
		global $db;
	...
	public function all_Completed_get($user_id){
		global $db;
	...
	public function all_Completed_p1($user_id){
		global $db;
